...
use Apartment\Db\Adapter\PostgresAdapter;
...
class PostgresAdapterTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @var \Apartment\Db\Adapter\PostgresAdapter
     */
    private $adapter;

    public function setUp()
    {
        if (!TESTS_Apartment_DB_ADAPTER_POSTGRES_ENABLED) {
            $this->markTestSkipped('Postgres tests disabled.  See TESTS_Apartment_DB_ADAPTER_POSTGRES_ENABLED constant.');
        }
        ...
        $options = [
            'host' => TESTS_Apartment_DB_ADAPTER_POSTGRES_HOST,
            'name' => TESTS_Apartment_DB_ADAPTER_POSTGRES_DATABASE,
            'user' => TESTS_Apartment_DB_ADAPTER_POSTGRES_USERNAME,
            'pass' => TESTS_Apartment_DB_ADAPTER_POSTGRES_PASSWORD,
            'port' => TESTS_Apartment_DB_ADAPTER_POSTGRES_PORT,
            'schema' => TESTS_Apartment_DB_ADAPTER_POSTGRES_DATABASE_SCHEMA
        ];
    ...
    {
        $this->adapter->connect();
        $table = new \Apartment\Db\Table($this->adapter->getSchemaTableName(), [], $this->adapter);
    ...
    {
        $table = new \Apartment\Db\Table('ntable', [], $this->adapter);
    ...
    {
        $table = new \Apartment\Db\Table('ntable', ['id' => 'custom_id'], $this->adapter);
        $table->addColumn('realname', 'string')
              ->addColumn('email', 'integer')
              ->save();
        $this->assertTrue($this->adapter->hasTable('ntable'));
    ...
    public function testCreateTableWithNoPrimaryKey()
    {
        ...
        $table = new \Apartment\Db\Table('atable', $options, $this->adapter);
    ...
    public function testCreateTableWithMultiplePrimaryKeys()
    {
        ...
        $table = new \Apartment\Db\Table('table1', $options, $this->adapter);
        $table->addColumn('user_id', 'integer')
    ...
    {
        $table = new \Apartment\Db\Table('table1', [], $this->adapter);
    ...
    {
        $table = new \Apartment\Db\Table('t', [], $this->adapter);
    ...
    public function testChangeColumn()
    {
        ...
        $table->addColumn('column1', 'string')
              ->save();
        $this->assertTrue($this->adapter->hasColumn('t', 'column1'));
        $newColumn1 = new \Apartment\Db\Table\Column();
        $newColumn1->setType('string');
        $table->changeColumn('column1', $newColumn1);
        $this->assertTrue($this->adapter->hasColumn('t', 'column1'));
        $newColumn2 = new \Apartment\Db\Table\Column();
    ...
    public function testDropIndex()
    {
        ...
        $table2 = new \Apartment\Db\Table('table2', [], $this->adapter);
        ...
        $table3 = new \Apartment\Db\Table('table3', [], $this->adapter);
        ...
        $table4 = new \Apartment\Db\Table('table4', [], $this->adapter);
    ...
    {
        $refTable = new \Apartment\Db\Table('ref_table', [], $this->adapter);
        $refTable->addColumn('field1', 'string')->save();

        $table = new \Apartment\Db\Table('table', [], $this->adapter);
    ...
    public function testHasDatabase()
    {
        ...
        $this->assertTrue($this->adapter->hasDatabase(TESTS_Apartment_DB_ADAPTER_POSTGRES_DATABASE));
    }

    public function testDropDatabase()
    {
        $this->assertFalse($this->adapter->hasDatabase('Apartment_temp_database'));
        $this->adapter->createDatabase('Apartment_temp_database');
        $this->assertTrue($this->adapter->hasDatabase('Apartment_temp_database'));
        $this->adapter->dropDatabase('Apartment_temp_database');
    }
    ...
    public function testGetApartmentType()
    {
        $this->assertEquals('integer', $this->adapter->getApartmentType('int'));
        $this->assertEquals('integer', $this->adapter->getApartmentType('int4'));
        $this->assertEquals('integer', $this->adapter->getApartmentType('integer'));

        $this->assertEquals('biginteger', $this->adapter->getApartmentType('bigint'));
        $this->assertEquals('biginteger', $this->adapter->getApartmentType('int8'));

        $this->assertEquals('decimal', $this->adapter->getApartmentType('decimal'));
        $this->assertEquals('decimal', $this->adapter->getApartmentType('numeric'));

        $this->assertEquals('float', $this->adapter->getApartmentType('real'));
        $this->assertEquals('float', $this->adapter->getApartmentType('float4'));

        $this->assertEquals('boolean', $this->adapter->getApartmentType('bool'));
        $this->assertEquals('boolean', $this->adapter->getApartmentType('boolean'));

        $this->assertEquals('string', $this->adapter->getApartmentType('character varying'));
        $this->assertEquals('string', $this->adapter->getApartmentType('varchar'));

        $this->assertEquals('text', $this->adapter->getApartmentType('text'));

        $this->assertEquals('time', $this->adapter->getApartmentType('time'));
        $this->assertEquals('time', $this->adapter->getApartmentType('timetz'));
        $this->assertEquals('time', $this->adapter->getApartmentType('time with time zone'));
        $this->assertEquals('time', $this->adapter->getApartmentType('time without time zone'));

        $this->assertEquals('datetime', $this->adapter->getApartmentType('timestamp'));
        $this->assertEquals('datetime', $this->adapter->getApartmentType('timestamptz'));
        $this->assertEquals('datetime', $this->adapter->getApartmentType('timestamp with time zone'));
        $this->assertEquals('datetime', $this->adapter->getApartmentType('timestamp without time zone'));

        $this->assertEquals('uuid', $this->adapter->getApartmentType('uuid'));
    }

    public function testCreateTableWithComment()
    {
        $tableComment = 'Table comment';
        $table = new \Apartment\Db\Table('ntable', ['comment' => $tableComment], $this->adapter);
        $table->addColumn('realname', 'string')
              ->save();
        $this->assertTrue($this->adapter->hasTable('ntable'));
        $this->assertTrue($this->adapter->hasColumn('ntable', 'id'));
    ...
    public function testCanAddColumnComment()
    {
        ...
        $row = $this->adapter->fetchRow(
            ...
            WHERE cols.table_catalog=\'' . TESTS_Apartment_DB_ADAPTER_POSTGRES_DATABASE . '\'
    ...
    {
        $table = new \Apartment\Db\Table('user', [], $this->adapter);
    ...
    {
        $table = new \Apartment\Db\Table('widgets', [], $this->adapter);
        $table->addColumn('transport', 'string', [
            'comment' => $comment = 'One of: car, boat, truck, plane, train'
            ])
            ->save();

        $table = new \Apartment\Db\Table('things', [], $this->adapter);
    ...
    public function testForignKeysArePropertlyEscaped()
    {
        ...
        $local = new \Apartment\Db\Table('users', ['primary_key' => $userId, 'id' => $userId], $this->adapter);
        $local->create();

        $foreign = new \Apartment\Db\Table('sessions', ['primary_key' => $sessionId, 'id' => $sessionId], $this->adapter);
    ...
    {
        $table = new \Apartment\Db\Table('tztable', ['id' => false], $this->adapter);
        $table
