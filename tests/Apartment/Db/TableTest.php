...
class TableTest extends \PHPUnit_Framework_TestCase
{
    public function provideTimestampColumnNames()
    {
        ...
        $adapters = array_filter(
            [
                TESTS_Apartment_DB_ADAPTER_SQLSRV_ENABLED ? new SqlServerAdapter([]) : false,
                TESTS_Apartment_DB_ADAPTER_MYSQL_ENABLED ? new MysqlAdapter([]) : false,
                TESTS_Apartment_DB_ADAPTER_POSTGRES_ENABLED ? new PostgresAdapter([]) : false,
                TESTS_Apartment_DB_ADAPTER_SQLITE_ENABLED ? new SQLiteAdapter([]) : false,
            ]
        );
    ...
    public function testAddColumnWithAnInvalidColumnType()
    {
        try {
            ...
            $table = new \Apartment\Db\Table('ntable', [], $adapter);
    ...
    {
        try {
            $table = new \Apartment\Db\Table('ntable');
    ...
    {
        $adapter = new MysqlAdapter([]);
        $table = new \Apartment\Db\Table('ntable', ['comment' => 'test comment'], $adapter);
    ...
    public function testAddIndex()
    {
        ...
        $this->assertEquals(\Apartment\Db\Table\Index::UNIQUE, $indexes[0]->getType());
    ...
    public function testAddIndexWithIndexObject()
    {
        ...
        $indexes = $table->getIndexes();
        $this->assertEquals(\Apartment\Db\Table\Index::INDEX, $indexes[0]->getType());
        $this->assertContains('email', $indexes[0]->getColumns());
    }
    ...
    {
        // stub adapter
        $adapterStub = $this->getMockBuilder('\Apartment\Db\Adapter\MysqlAdapter')
            ->setConstructorArgs([[]])
            ->getMock();
        $adapterStub->expects($this->once())
                    ->method('changeColumn');
        $newColumn = new \Apartment\Db\Table\Column();
        $table = new \Apartment\Db\Table('ntable', [], $adapterStub);
    ...
    public function testGetColumns()
    {
        ...
        $table = new \Apartment\Db\Table('table1', [], $adapterStub);
        $table->getColumns();
    }
